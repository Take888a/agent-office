...
- **実機(ブラウザ)での見た目・動作確認はオーナーが行う。** Claude はブラウザ
  自動化での確認はせず、コード側の検証(`npx tsc --noEmit` / lint / build)まで
  行ったら変更内容と確認ポイントを伝えて引き渡すこと。
- オーダーを出さなければ実エージェントは起動しない(常駐社員は休憩表示のみ)ので、
  見た目の確認はトークン消費ゼロでできる。
